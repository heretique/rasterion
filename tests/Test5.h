...
public:
  ...
  Test5(int width, int height) : _width(width), _height(height) {}

  virtual void init() override {
    std::string inputfile = "assets/diablo3_pose.obj";

    std::string err;
    if (tinyobj::LoadObj(&_attrib, &_shapes, &_materials, &err,
                         inputfile.c_str())) {
    ...
    for (size_t s = 0; s < _shapes.size(); s++) {
      ...
      for (size_t f = 0; f < _shapes[s].mesh.num_face_vertices.size(); f++) {
        ...
        for (size_t v = 1; v < fv; v++) {
          ...
          rs::DrawLine(p0.x, p0.y, p1.x, p1.y, {0xff, 0x0, 0x0, 0x0});
